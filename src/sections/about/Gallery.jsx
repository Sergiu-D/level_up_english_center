...
export default function Gallery() {
  ...
  useEffect(() => {
    ...
    if (galleryItemRefs.current.length > 0) {
      galleryItemRefs.current.forEach((ref, index) => {
        if (ref) {
          inView(ref, () => {
  ...
  useEffect(() => {
    if (selectedImage && lightboxRef.current) {
      animate(lightboxRef.current, { opacity: [0, 1], scale: [0.9, 1] }, { duration: 0.3 });
    }
  }, [selectedImage]);
  
  // Reset refs when component updates
  useEffect(() => {
    galleryItemRefs.current = galleryItemRefs.current.slice(0, 6); // We have 6 gallery images
  }, []);
      ...
      <div className="container mx-auto px-4">
        ...
        {selectedImage && (
          ...
          >
            <div
              ref={lightboxRef}
              className="relative max-h-[80vh] max-w-4xl opacity-0 scale-90"
