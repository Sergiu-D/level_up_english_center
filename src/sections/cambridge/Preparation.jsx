...
export default function Preparation() {
  ...
  useEffect(() => {
    ...
    // Steps animation
    if (stepsRefs.current.length > 0) {
      stepsRefs.current.forEach((stepRef, index) => {
        if (stepRef) {
          inView(stepRef, () => {
            animate(stepRef, { opacity: [0, 1], y: [20, 0] }, { duration: 0.5, delay: index * 0.1 });
            return () => {};
          });
        }
      });
  ...
  // Reset steps refs when steps change
  useEffect(() => {
    stepsRefs.current = stepsRefs.current.slice(0, preparationSteps.length);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {preparationSteps.map((step, index) => (
            <div
              ...
              className="bg-gradient-to-br from-blue-50 to-purple-50 p-6 rounded-xl border border-gray-100 hover:shadow-md transition-shadow opacity-0"
            >
