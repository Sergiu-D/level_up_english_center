...
export default function ContactForm() {
  ...
  useEffect(() => {
    ...
    if (formContainerRef.current) {
      inView(formContainerRef.current, () => {
        animate(formContainerRef.current, { opacity: [0, 1], x: [-20, 0] }, { duration: 0.6 });
        return () => {};
      });
    }
  }, []);

  // Status message animation when form is submitted
  useEffect(() => {
    if (formStatus.submitted && statusMessageRef.current) {
      animate(statusMessageRef.current, { opacity: [0, 1], y: [-10, 0] }, { duration: 0.3 });
    }
    ...
    <section id="contact-form" className="py-16 md:py-24 bg-white">
      <div className="container mx-auto px-4 md:px-8 lg:px-16">
        <div
          ...
          className="max-w-xl mx-auto md:mx-0 md:ml-auto opacity-0"
        >
          ...
          {formStatus.submitted && (
            <div
              ...
              className={`mb-6 p-4 rounded-lg opacity-0 ${formStatus.success ? 'bg-green-50 text-green-800' : 'bg-red-50 text-red-800'}`}
            >
              {formStatus.message}
            </div>
          )}
